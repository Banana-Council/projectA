...
class TrackNewFileHandler:
    ...
    def execute(self, command: TrackNewFileCommand) -> Result[None, Exception]:
        ...
        tracking_file = self.create_tracking_file(command.alias, command.tracked_file_path)
    ...
    def create_tracking_file(self, alias: str, tracked_file_path: str) -> str:
        tracking_directory = os.path.expanduser(f"~/{APP_NAME}/{alias}/")
        tracking_file_path = os.path.join(tracking_directory, os.path.basename(tracked_file_path))
